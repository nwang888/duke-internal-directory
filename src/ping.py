...
for csv_file in csv_files:
    ...
    with open(DATA_PATH + csv_file[:-4] + 'OUT.csv', 'w', encoding='utf-8-sig') as f:
        writer = csv.DictWriter(f, fieldnames=['netID', 'valid', 'noInfo', 'noGradTerm', 'invalidYear', 'noProgram', 'invalidProgram'], lineterminator="\n")
        writer.writeheader()
        ...
        for netid in netids:
            ...
            noInfo = directory_details == None
            if noInfo:
                isValid = False
            else: 
                noGradTerm = 'Graduation Term' not in directory_details.keys()
                invalidYear = directory_details['Graduation Term'] not in YEARS
                noProgram = 'Program' not in directory_details.keys()
                invalidProgram = directory_details['Program'] not in PROGRAMS
                if (noInfo or noGradTerm or invalidYear or noProgram or invalidProgram):
                    isValid = False
            if isValid:
                writer.writerow({'netID': netid, 'valid': 'True'})
            else:
                writer.writerow({'netID': netid, 'valid': 'False', 'noInfo': noInfo, 'noGradTerm': noGradTerm, 'invalidYear': invalidYear, 'noProgram': noProgram, 'invalidProgram': invalidProgram})
        print('done')